...
template <typename T>
cudaError_t keep_m_in_n<T>::launch(const T *in, T *out, int m, int window, int N, int grid_size,
...
template <>
cudaError_t keep_m_in_n<std::complex<double>>::launch(
...
template <>
cudaError_t keep_m_in_n<std::complex<double>>::occupancy(int *minBlock, int *minGrid) {
  return cudaOccupancyMaxPotentialBlockSize(minGrid, minBlock,
