...
template <typename T> __global__ void kernel_mag_squared(const T *in, T *out, int N) {
  ...
  if (i < N) {
    out[i].x = powf(in[i].x, 2) + powf(in[i].y, 2);
    out[i].y = 0;
  }
}
...
cudaError_t complex_to_mag_squared<T>::launch(const T *in, T *out, int N, int grid_size, int block_size,
                  cudaStream_t stream) {
  if (stream) {
    kernel_mag_squared<<<grid_size, block_size, 0, stream>>>(in, out, N);
  } else {
    kernel_mag_squared<<<grid_size, block_size>>>(in, out, N);
  }
...
template <typename T> cudaError_t complex_to_mag_squared<T>::occupancy(int *minBlock, int *minGrid) {
  return cudaOccupancyMaxPotentialBlockSize(minGrid, minBlock, kernel_mag_squared<T>, 0, 0);
}
