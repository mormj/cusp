...
template <typename T> void get_block_and_grid(int* minGrid, int* minBlock) {
    cudaOccupancyMaxPotentialBlockSize(minGrid, minBlock, kernel_or<T>, 0, 0);
}
...
void launch_kernel_or(const T **in, T *out, int grid_size, int block_size,
  ...
  if (stream) {
    kernel_or<<<grid_size, block_size, 0, stream>>>(in, out, num_inputs, N);
  } else {
    kernel_or<<<grid_size, block_size>>>(in, out, num_inputs, N);
  }
}
