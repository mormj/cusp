...
#include "../include/cusp/multiply.cuh"
#include <iostream>
...
template <>
__global__ void kernel_multiply<thrust::complex<float>>(const thrust::complex<float> **ins,
...
template <typename T>
cudaError_t multiply<T>::launch(const std::vector<const void *> inputs, T *output,
  ...
  if (stream) {
    kernel_multiply<<<grid_size, block_size, 0, stream>>>((const T **)_dev_ptr_array,
                                                     output, ninputs, nitems);
  } else {
    kernel_multiply<<<grid_size, block_size>>>((const T **)_dev_ptr_array, output,
                                          ninputs, nitems);
  }
...
template <>
cudaError_t multiply<std::complex<float>>::launch(const std::vector<const void *> inputs,
  ...
  if (stream) {
    kernel_multiply<<<grid_size, block_size, 0, stream>>>((const thrust::complex<float> **)_dev_ptr_array,
                                                     (thrust::complex<float> *)output, ninputs, nitems);
  } else {
    kernel_multiply<<<grid_size, block_size>>>((const thrust::complex<float> **)_dev_ptr_array,
...
cudaError_t multiply<T>::occupancy(int *minBlock, int *minGrid) {
  return cudaOccupancyMaxPotentialBlockSize(minGrid, minBlock, kernel_multiply<T>, 0,
                                            0);
}

template <>
cudaError_t multiply<std::complex<float>>::occupancy(int *minBlock, int *minGrid) {
  return cudaOccupancyMaxPotentialBlockSize(minGrid, minBlock,
                                            kernel_multiply<thrust::complex<float>>, 0, 0);
}
