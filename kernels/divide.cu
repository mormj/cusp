...
template <>
__global__ void
kernel_divide<thrust::complex<float>>(const thrust::complex<float> **ins,
                                      thrust::complex<float> *out, int ninputs,
                                      int N) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < N) {
...
template <typename T>
__global__ void kernel_divide2(const T *in1, const T *in2, T *out,
                                 int ninputs, int N) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < N) {
    out[i] = in1[i] / in2[i];
...
template <>
__global__ void kernel_divide2<thrust::complex<float>>(
...
cudaError_t divide<T>::launch(const std::vector<const void *> &inputs,
  ...
  if (ninputs == 2) {
    if (stream) {
      kernel_divide2<<<grid_size, block_size, 0, stream>>>(
          (const T *)inputs[0], (const T *)inputs[1], (T *)output, ninputs,
          nitems);
    } else {
      kernel_divide2<<<grid_size, block_size>>>((const T *)inputs[0],
                                                  (const T *)inputs[1],
  ...
  if (stream) {
    kernel_divide<<<grid_size, block_size, 0, stream>>>(
...
template <>
cudaError_t
divide<std::complex<float>>::launch(const std::vector<const void *> &inputs,
  ...
  if (ninputs == 2) {
    if (stream) {
      ...
      kernel_divide2<<<grid_size, block_size>>>(
          (const thrust::complex<float> *)inputs[0],
          (const thrust::complex<float> *)inputs[1],
          (thrust::complex<float> *)output, ninputs, nitems);
    }
  ...
  if (stream) {
    ...
    kernel_divide<<<grid_size, block_size>>>(
        (const thrust::complex<float> **)_dev_ptr_array,
        (thrust::complex<float> *)output, ninputs, nitems);
  }
  return cudaPeekAtLastError();
...
cudaError_t divide<T>::occupancy(int *minBlock, int *minGrid) {
  return cudaOccupancyMaxPotentialBlockSize(minGrid, minBlock, kernel_divide<T>,
                                            0, 0);
}

template <>
cudaError_t divide<std::complex<float>>::occupancy(int *minBlock,
                                                   int *minGrid) {
  return cudaOccupancyMaxPotentialBlockSize(
      minGrid, minBlock, kernel_divide<thrust::complex<float>>, 0, 0);
}
