...
template <typename T>
__global__ void kernel_and(const T **ins, T *out, int ninputs, int N) {
...
template <typename T>
cudaError_t and_bitwise<T>::launch(const std::vector<const void *> inputs, T *output,
  ...
  if (stream) {
    kernel_and<<<grid_size, block_size, 0, stream>>>((const T **)_dev_ptr_array,
                                                     output, ninputs, nitems);
  } else {
    kernel_and<<<grid_size, block_size>>>((const T **)_dev_ptr_array, output,
                                          ninputs, nitems);
  }
...
template <typename T>
cudaError_t and_bitwise<T>::launch(const std::vector<const void *> inputs,
...
template <typename T>
cudaError_t and_bitwise<T>::occupancy(int *minBlock, int *minGrid) {
  return cudaOccupancyMaxPotentialBlockSize(minGrid, minBlock, kernel_and<T>, 0,
                                            0);
}

#define IMPLEMENT_KERNEL(T) template class and_bitwise<T>;
