...
// this code should be autogenerated by the modtool -> third parameter
// is name of generated function
...
void launch_kernel_and(const T *in, T *out, int grid_size, int block_size,
  ...
  if (stream) {
    kernel_and<<<grid_size, block_size, 0, stream>>>(in1, in2, out, N);
  } else {
    kernel_and<<<grid_size, block_size>>>(in1, in2, out, N);
  }
}
