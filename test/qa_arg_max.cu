...
void run_test(int N, int num_inputs)
{
    ...
    for (int i = 0; i < N; i++) {
      host_input_data[i] = (T)(i + 5);
    }
    expected_output_data[0] = std::max_element(host_input_data.begin(),
                                               host_input_data.end()) - 
...
TEST(ArgMaxKernel, Basic) {
  int N = 1024 * 100;

  run_test<int32_t>(N, 1);
  run_test<int64_t>(N, 2);
  run_test<float>(N, 3);
  run_test<double>(N, 4);
}